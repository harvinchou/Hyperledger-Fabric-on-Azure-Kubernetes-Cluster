...
secret_exists=$(az keyvault secret list --vault-name ${keyVaultName} --query "contains([].id, 'https://${keyVaultName}.vault.azure.cn/secrets/${secretName}')")
needs_updating=true
...
#copy deploy.yaml to deployment_artifacts folder
cp $CHIANCODE_SHARED_ROOT/deploy.yaml $MANIFEST_FILE_PATH
